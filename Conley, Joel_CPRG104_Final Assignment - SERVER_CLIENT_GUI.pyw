...
class serverFrontend(Tk):
	...
	class GUI_object(Tk):
		...
		def specs(obj, _type_, text, font, state, row, rowspan, col, colspan, padx, pady, ipadx, ipady, sticky):
			...
			if obj.type == "Button":
				class_button = Button(obj,  text=obj.text, font=obj.font, width=6, state=obj.state, background="ivory3").grid(row=obj.row, rowspan=obj.rowspan, column = obj.col, columnspan=obj.colspan, padx=obj.padx, pady=obj.pady, ipadx=obj.ipadx, ipady=obj.ipady, sticky=obj.sticky)
	...
	def __init__(UI):
		...
		# UI._canvas = Canvas(UI, width = 450, height = 450, bg ='blue', highlightthickness = 0)
		# UI._canvas.create_line(1, 3, 450, 0, width = 5.0, fill="red")
